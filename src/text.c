...
static u16 FontFunc_Palooka(struct TextPrinter *);
...
static void DecompressGlyph_Palooka(u16, bool32);
...
static u32 GetGlyphWidth_Palooka(u16, bool32);
...
static const struct GlyphWidthFunc sGlyphWidthFuncs[] =
{
    ...
    { FONT_PALOOKA,        GetGlyphWidth_Palooka},
};
...
static u16 FontFunc_Palooka(struct TextPrinter *textPrinter)
{
    if (textPrinter->hasFontIdBeenSet == FALSE)
    {
        textPrinter->fontId = FONT_PALOOKA;
        textPrinter->hasFontIdBeenSet = TRUE;
    }
    return RenderText(textPrinter);
}
...
static u16 RenderText(struct TextPrinter *textPrinter)
{
    ...
    case RENDER_STATE_HANDLE_CHAR:
        ...
        case FONT_SHORT_NARROWER:
            ...
            break;
        case FONT_PALOOKA:
            DecompressGlyph_Palooka(currChar, textPrinter->japanese);
            break;
        case FONT_BRAILLE:
            break;
        }
...
static void DecompressGlyph_Palooka(u16 glyphId, bool32 isJapanese)
{
    const u16 *glyphs;

    glyphs = gFontPalookaGlyphs + (0x20 * glyphId);
    gCurGlyph.width = gFontPalookaGlyphWidths[glyphId];
...
static u32 GetGlyphWidth_Palooka(u16 glyphId, bool32 isJapanese)
{
    return gFontPalookaGlyphWidths[glyphId];
}
